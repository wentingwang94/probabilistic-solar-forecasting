...
###############################################################################
# load ECMWF ENS data
# BON:  Latitude: 40.05192, Longitude: -88.37309 
# Time zone: UTC-6 (Etc/GMT+6)
metadata1 = pd.read_csv('C:/Users/81095/PY/ENS/BON_2017_2018.csv')
metadata2 = pd.read_csv('C:/Users/81095/PY/ENS/BON_2019_2020.csv')
# Define a new dataframe: "data_BON", which stores ensemble forecasts of 50 members.
data_BON = pd.DataFrame(columns=[f'EC_GHI_{x+1}' for x in range(50)])
for i in range(50):
    ...
    # loop assignment.
    data_BON[f'EC_GHI_{i+1}'] = ghi_df['ghi']
...
################################################################################
# convert time zone
Times = pd.date_range(start = '2016-01-01 07:00:00',end='2021-01-01 06:00:00',freq = '1h')
nsrdb['UTC'] = Times
nsrdb['UTC-6'] = nsrdb.index
nsrdb.index = range(len(nsrdb))
# index number "8778" is UTC: 2017-01-01 01:00:00
# index number "43840" is UTC: 2020-12-31 23:00:00
nsrdb_target = nsrdb.iloc[8778:43841,:]
nsrdb_target = nsrdb_target.rename(columns={'GHI':'NSRDB_GHI'})
nsrdb_target.index = range(len(nsrdb_target))

################################################################################
# establish target data
data_target = pd.concat([data_BON,nsrdb_target],axis=1) 
...
# train
data_train = data_target.iloc[:12206,:]
data_train.index = range(len(data_train))
# test
data_test = data_target.iloc[12206:,:]
...
# bounds = Bounds([0.001,0.001],[10,10])
# x0 = np.array([0.6,0.5])
# res = minimize(objective, x0, method='SLSQP', bounds=bounds, options={'ftol': 1e-9, 'maxiter': 200,  'disp': True})
