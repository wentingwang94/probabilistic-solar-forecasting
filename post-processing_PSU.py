...
###############################################################################
# load ECMWF ENS data
# PSU:  Latitude: 40.72012, Longitude: -77.93085 
# Time zone: UTC-6 (Etc/GMT+6)
metadata1 = pd.read_csv('C:/Users/81095/PY/ENS/PSU_2017_2018.csv')
metadata2 = pd.read_csv('C:/Users/81095/PY/ENS/PSU_2019_2020.csv')
# Define a new dataframe: "data_PSU", which stores ensemble forecasts of 50 members.
data_PSU = pd.DataFrame(columns=[f'EC_GHI_{x+1}' for x in range(50)])
...
for i in range(50):
    ...
    # loop assignment.
    data_PSU[f'EC_GHI_{i+1}'] = ghi_df['ghi']
...
################################################################################
# convert time zone
Times = pd.date_range(start = '2016-01-01 06:00:00',end='2021-01-01 05:00:00',freq = '1h')
nsrdb['UTC'] = Times
nsrdb['UTC-5'] = nsrdb.index
nsrdb.index = range(len(nsrdb))
# index number "8779" is UTC: 2017-01-01 01:00:00
# index number "43841" is UTC: 2020-12-31 23:00:00
nsrdb_target = nsrdb.iloc[8779:43842,:]
nsrdb_target = nsrdb_target.rename(columns={'GHI':'NSRDB_GHI'})
nsrdb_target.index = range(len(nsrdb_target))

################################################################################
# establish target data
data_target = pd.concat([data_PSU,nsrdb_target],axis=1) 
...
# train
data_train = data_target.iloc[:12175,:]
data_train.index = range(len(data_train))
# test
data_test = data_target.iloc[12175:,:]
